...
`hostsToExclude` (optional):: A list of hosts that should be excluded from the sanity check. These hosts can be written as regular expressions.
+
Type: List.
+
Default: Empty list
...
htmlSanityCheck {
   ...
   // http connection timeout in milliseconds
    ...
    urlsToExclude = ['(http|https)://example.com/.*', "http://another-example.com/skip"]

    // Hosts to exclude from checking
    hostsToExclude = ["exclude-host.com", "skip-host.org"]
