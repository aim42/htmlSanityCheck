...
* xref:chap-08-Concepts.adoc#checker_blackbox[CheckerCore] shall encapsulate checking logic and Html parsing/processing.
...
|===
| <<checker_blackbox, HSC Core>> | hsc core: html parsing and sanity checking, configuration, reporting.
...
| usage via shell
| an (arc42) user uses a command line shell to call kbd:[HtmlSC]

| Buildsystem
| Currently restricted to Gradle: The build system uses kbd:[HtmlSC] as configured in the buildscript.

| Local filesystem
| kbd:[HtmlSC] needs access to several local files, especially the html page to be checked and to the corresponding image directories.

| External websites
| to check external links, kbd:[HtmlSC] needs to access external sites via http HEAD or GET requests.
