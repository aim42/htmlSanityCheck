...
| local-file system | kbd:[HtmlSC] needs access to several
local files, especially the html page to be checked and to the
corresponding image directories.
| external websites| to check external links, kbd:[HtmlSC] needs to access external sites via http HEAD or GET requests.
|===
