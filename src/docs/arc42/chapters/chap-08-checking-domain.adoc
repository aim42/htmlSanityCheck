...
* Another part of this document (->Internal Link) or
* Another document, image or resource (can be either ->Internal (local) or ->External Link).
Every link leads from the _Link Source_ to the _Link Target_

| Link Target
| The target of any ->Link, e.g., heading or any other part of an ->HTML Document, any internal or external resource (identified by URI). Expressed by ->id
...
Defined in https://www.ietf.org/rfc/rfc2396.txt[RFC-2396]. The ultimate source of truth concerning link syntax and semantic.
|===
