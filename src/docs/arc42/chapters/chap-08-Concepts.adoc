...
HSC allows for different output formats and destinations:

* Formats (HTML and text) and
* Destinations (file and console)

The reporting subsystem uses the template method pattern to allow different output formats (e.g., Console and HTML).
The overall structure of reports is always the same:
...
<4> Write report footer - in HTML report also creates a back-to-top-link
...
The appropriate copy method took a https://github.com/gradle/gradle/blob/5c45052ef3fa9395fa85f71340611bd159cd1489/testing/internal-performance-testing/src/main/groovy/org/gradle/performance/results/report/AbstractReportGenerator.java#L190[Gradle source] as a blueprint.
