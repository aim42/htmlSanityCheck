...
Checks if image files referenced in +<img src="someFile.jpg">+ really exist on the local file system.

The (little) problem with checking images is their path: Consider the following HTML fragment:
...
Only links with the prefix +#+ shall be taken into account, e.g. +<a href="#internalLink">+.
...
Problem here are networking issues, latency and HTTP return codes.
