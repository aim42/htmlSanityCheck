...
In 2024/2025 we decided to port HSC to plain Java, because in some environments the use of the Groovy-framework caused trouble.
In some cases different versions of the Groovy environment used in Maven projekts resulted in an incompatibility that renders the build not working.
Using plain Java in the core library and the Maven plugin, we get rid of all Groovy dependencies and therefore eliminate all version conflicts.
